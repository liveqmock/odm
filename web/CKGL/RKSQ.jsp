...
<head>
    ...
    <script>
        function cgrukun(cgid)
        {
            location.href = "DisPatch_getAddJsp?a=2&b=10&cgid="+cgid;
            return true;
        }
        function sczjgruku(cgid)
        {
            location.href = "DisPatch_getAddJsp?a=2&b=11&sczjgid="+cgid;
            return true;
        }
    </script>
</head>
    ...
    <table cellpadding="0" cellspacing="0" width="100%" align="center"
        ...
        <tr>
            ...
            <td width="140" class="td2"><select name="zhijiantype"
                ...
                <option value="自加工入库">自加工入库</option>
                <option value="外加工入库">外加工入库</option>
            ...
            <td width="140" class="td2">
                <select name="type_num"
                        id="type_num" class="a2" style="position: relative" onchange="clearID();">
                    <option value="<%=PublicFunc.ORDER_STATE_WATINGRK%>" ><%=PublicFunc.ORDER_STATE_WATINGRK%></option>
                    <option value="<%=PublicFunc.ORDER_STATE_WATINGZJ%>" ><%=PublicFunc.ORDER_STATE_WATINGZJ%></option>
                    <option value="采购中/生产中" >采购中/生产中</option>
                    <option value="<%=PublicFunc.ORDER_STATE_FINISH%>" ><%=PublicFunc.ORDER_STATE_FINISH%></option>
                    <option value="--请选择--" >全部</option>

                </select>
            </td>
        </tr>
        <input type="hidden" name="statedefault" id="statedefault" value=<%=PublicFunc.ORDER_STATE_WATINGRK%> />
        <tr>
...
<script>
    function clearID()
    {
        ...
        ss1.value = "";
      //  ss2.value = document.getElementById("statedefault").value;
        ss3.value = "";
        ss4.value = "";
        gridFlash();
    }
    ...
    $("#flex1").flexigrid({
        url : 'SCGL_dolist?hidden=manage&type=RUKULIST',
        dataType : 'json',
        colModel : [ {
            display : 'id',
            name : 'id',
...
<script>
    <%
            String message = (String)request.getAttribute("cgrkerror");
             if (message != null && !"".equals(message)) {
                 request.removeAttribute("cgrkerror");
         %>
