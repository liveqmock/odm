...
<body>
	<div>
			<table border="0" cellpadding="0" cellspacing="0" width="100%"
				id="tabs">
				<tr >
					<td class="tabs"  align="center" ><font color="white" >半成品资料</font>
						</td>
					<td class="tabs"></td>
				</tr>
			</table>
...
function gridFlash(){  
	$("#flex1").flexOptions({params: [
                                  ...
                                  {name:'xilie',value:$("#xilie").val()}
                                  ]}).flexReload();
} 
	$("#flex1").flexigrid({

		url : 'CGGL_dolist?hidden=manage&type=ZLGL_BCP',
		dataType : 'json',
		colModel : [ {
			display : 'id',
			name : 'id',
		...
		if (com == '删除') {
			...
			if ($('.trSelected', grid).length == 0) {
				...
				if (confirm('是否删除这 ' + $('.trSelected', grid).length + ' 条记录吗?')) {
					...
					var url = "CGGL_dolist?hidden=delete&type=ZLGL_BCP&" + id;
					del(url);
				}
			}
		} else if (com == '添加') {
			hidden.value = "add";
			location.href = "DisPatch_getAddJsp?a=1&b=4";
		} else if (com == '修改') {
			 hidden.value="modify";  
	          if($(".trSelected").length==1){   
	        	  window.location.href =  "CGGL_dolist?hidden=premodify&type=ZLGL_BCP&id="+$('.trSelected', grid).find("td").eq(0).text();
