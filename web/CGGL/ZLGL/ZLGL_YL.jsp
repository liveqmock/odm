...
<body>
	<div>
			<table border="0" cellpadding="0" cellspacing="0" width="100%"
				id="tabs">
				<tr >
					<td class="tabs"  align="center" ><font color="white" >原料资料</font>
						</td>
					<td class="tabs"></td>
				</tr>
			</table>
		...
		if (com == '删除') {
			...
			if ($('.trSelected', grid).length == 0) {
				...
				if (confirm('是否删除这 ' + $('.trSelected', grid).length + ' 条记录吗?')) {
					...
					var url = "CGGL_dolist?hidden=delete&type=ZLGL_YL&" + id;
					del(url);
				}
			}
		} else if (com == '添加') {
			hidden.value = "add";
			location.href = "DisPatch_getAddJsp?a=1&b=3";
		} else if (com == '修改') {
			  hidden.value="modify";  
	          if($(".trSelected").length==1){   
	        	  window.location.href =  "CGGL_dolist?hidden=premodify&type=ZLGL_YL&id="+$('.trSelected', grid).find("td").eq(0).text();
