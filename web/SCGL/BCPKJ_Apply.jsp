...
<div>
	<table border="0" cellpadding="0" cellspacing="0" width="100%"
				id="tabs">
				<tr >
					<td class="tabs"  align="center" ><font color="white" >半成品开剪申请</font>
						</td>
					<td class="tabs"></td>
				</tr>
			</table>
	...
	function button(com, grid) {
		...
		if (com == '标记为无效') {
            ...
            if ($(".trSelected").length == 1) {
                ...
                var url =  "SCGL_dolist?hidden=disable&type=BCPKJAPPLY&id="+$('.trSelected', grid).find("td").eq(0).text();
                del(url);
			...
			hidden.value = "add";
			location.href = "DisPatch_getAddJsp?a=2&b=6";
		}
        else if (com == '修改') {
			hidden.value = "modify";
			if ($(".trSelected").length == 1) {
				window.location.href =  "SCGL_dolist?hidden=premodify&type=BCPKJAPPLY&id="+$('.trSelected', grid).find("td").eq(0).text();
			} else if ($(".trSelected").length > 1) {
