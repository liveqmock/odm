...
<div>
		<table border="0" cellpadding="0" cellspacing="0" width="100%"
			id="tabs">
			<tr>
				<td class="tabs" align="center"><font color="white">查询自加工任务单</font>
				</td>
				<td class="tabs"></td>
			</tr>
		</table>
			...
			<td width="132" align="center" class="td1">任务单号</td>
				...
				<td width="140" class="td2"><select name="zhuangtai"
					...
					String states[] = { PublicFunc.ORDER_STATE_NOPERSON,PublicFunc.ORDER_STATE_RESEND,
							PublicFunc.SC_STATE_WATINGSC,PublicFunc.ORDER_STATE_WATINGRK,
		...
		if (com == '删除') {
			...
			if ($('.trSelected', grid).length == 0) {
				...
				comstr = id;
				 ...
				 if($('.trSelected', grid).find("td").eq(7).text() == orderstate5.value)
				 {
					 //未打印前不用输密码。。
			...
			if ($(".trSelected").length == 1) {
				 ...
				 getstatusbycgid("SCGL_dolist?hidden=premodify&type=SCGLZJG&id="+$('.trSelected', grid).find("td").eq(0).text());
