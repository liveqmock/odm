...
<table border="0" cellpadding="0" cellspacing="0" width="100%"
			id="tabs">
			<tr>
				<td class="tabs" align="center"><font color="white">外加工任务单管理</font>
				</td>
				<td class="tabs"></td>
			</tr>
		</table>
	...
	<div>
		<table cellpadding="0" cellspacing="0" width="100%" align="center"
			...
			<tr>
				<td width="132" align="center" class="td1">加工单位名称</td>
				<td width="140" class="td2"><input type="text" name="JG_danwei" id="JG_danwei"/>
				</td>
			...
			<tr>
				...
				<td width="140" class="td2"><select name="gongyi"
                    ...
                    <option value="<%=PublicFunc.wjgGYType[i] %>"   ><%=PublicFunc.wjgGYType[i] %></option>
                    <%} %>
				</select></td>
...
function gridFlash(){  
	$("#flex1").flexOptions({params: [
	                                  {name:'JG_danwei', value:$("#JG_danwei").val()},
	...
	$("#flex1").flexigrid({
		url : 'SCGL_dolist?hidden=manage&type=SCGLWJG',
		dataType : 'json',
		colModel : [ {
			display : 'id',
			name : 'id',
...
var comstr;
		...
		if (com == '删除') {
			...
			if ($('.trSelected', grid).length == 0) {
				...
				if (confirm('是否删除这 ' + $('.trSelected', grid).length + ' 条记录吗?')) {
					...
					var url = "SCGL_dolist?hidden=delete&type=SCGLWJG&" + id;
					del(url);
				}
			}
		}  else if (com == '添加') {
			hidden.value = "add";
			location.href = "DisPatch_getAddJsp?a=2&b=1";
		} else if (com == '修改') {
            hidden.value = "modify";
            if ($(".trSelected").length == 1) {
                ...
                if ($('.trSelected', grid).find("td").eq(9).text() == orderstate1.value
                        || $('.trSelected', grid).find("td").eq(9).text() == orderstate2.value
                        || $('.trSelected', grid).find("td").eq(9).text() == orderstate3.value
                        || $('.trSelected', grid).find("td").eq(9).text() == orderstate4.value
                        || $('.trSelected', grid).find("td").eq(9).text() == orderstate6.value
                        ) {
                    alert("当前任务状态，不支持该操作!");
                    return;
                }
                ...
                if ($('.trSelected', grid).find("td").eq(9).text() == orderstate5.value
                        || $('.trSelected', grid).find("td").eq(9).text() == orderstate7.value) {
                    //未打印前不用输密码。。
                ...
                getstatusbycgid("DisPatch_modifyWJG?id=" + $('.trSelected', grid).find("td").eq(0).text());
            ...
            if ($(".trSelected").length == 1) {
                ...
                if (confirm("是否确定置为无效？")) {
                    var x = "DisPatch_disableSCWJG?id=" + $('.trSelected', grid).find("td").eq(0).text();
                    disableline(x);
                } else {
                    return;
                }
    ...
    function newaction() {
        window.location.href = "SCGL_dolist?hidden=premodify&type=SCGLWJG&id=" + comstr;
    }
</script>
</html>
