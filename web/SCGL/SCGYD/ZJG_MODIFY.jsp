...
<head>
    ...
    <script type="text/javascript">
        ...
        function backMenu() {
            ...
            f.action = "DisPatch_getUrl?i=0&j=2";
            //f.target = "mainframe";
            f.method = "post";
...
<%
    ...
    String jgtype ="--请选择--";
    int id = -1;
    if(null != request.getAttribute("zjg"))
    {
        ZJGgongyidan xh = (ZJGgongyidan)request.getAttribute("zjg");
        ...
        if(xh.getChejian() != null)
        {
            chejian = xh.getChejian() ;
        }
    ...
    String chejiansession = (String)session.getAttribute("zjgchejian");
%>
<form id="form1" name="form1" method="post"
      action="SCGL_dolist?hidden=modify&type=SCGLZJG">
<input type="hidden" name="id" value="<%=id%>"/>
<div>
    <table border="0" cellpadding="0" cellspacing="0" width="100%"
           id="tabs">
        <tr>
            <td class="tabs" align="center"><font color="white">添加自加工工艺单</font>
            </td>
            <td class="tabs"></td>
        </tr>
    </table>
                ...
                <table  cellSpacing=0 cellPadding=0 width=600
                    ...
                    <tr>
                        ...
                        <td  ><select style="position: relative;" onchange="enableselect()"
                            ...
                            %>
                            <OPTION value="<%=values[i]%>" <%if (values[i].equals(jgtype)) {%> selected <%} %> ><%=values[i]%></OPTION>
                            <%}%>
                        </select></td>
                        <td id="zgmmerror">&nbsp;</td>
                    </tr>
                            ...
                            <div id="hide1" style="display:none">
                                <select  class="flexselect"  id="xinghao1"
                                    ...
                                    <%
                                        ...
                                        if (null != session.getAttribute("bcp_cp_type_nums")) {
                                            strs = (Map<String, List<String>>) session.getAttribute("bcp_cp_type_nums");
                                        }
                                        List<String> str1 = strs.get("str1");
                                        for(int i=0;i<str1.size();i++)
                                        {
                                    %>
                                    <option <%if(str1.get(i).equals(jgxinghao)) {%> selected<%}%> ><%=str1.get(i) %></option>
                                    <%
                                        }
                                    %>
                                </select></div>
                            <div id="hide2"   style="display:none">
                                <select class="flexselect"  id="xinghao2"
                                    ...
                                    <%
                                        ...
                                        {
                                    %>
                                    <option <%if(str2.get(i).equals(jgxinghao)) {%> selected<%}%> ><%=str2.get(i) %></option>
                                    <%
                                        }
                                    %>
                                </select>
                        ...
                        <td>
                            <select  class="flexselect"  id="xinghao"
                                ...
                                <%
                                    ...
                                    {
                                %>
                                <option <%if(str1.get(i).equals(xinghao)) {%> selected<%}%> ><%=str1.get(i) %></option>
                                <%
                                    }
                                %>
                            </select>
                    ...
                    <tr>
                        <td class=td3 align="center" style="width:10%">车间</td>
                        <td width="200px">
                            <input type="text" size="24" id="chejian" value="<%=chejiansession%>"
                                   onfocus="this.blur()" readonly="readonly"  style="color:gray"
                                   name="chejian" maxlength="24"/>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        ...
                        <td width="200px"><select name="gongyi"
                            ...
                            <option value="<%=PublicFunc.zjgGYType[i] %>" <%if(gongyi.equals(PublicFunc.zjgGYType[i])){ %>
                                    selected<%}%> ><%=PublicFunc.zjgGYType[i] %></option>
                            <%} %>
                        </select>
                        </td>
                        <td>&nbsp;</td>
...
<script>
    <%
            String message = (String)request.getAttribute("modifyzjgerror");
             if (message != null && !"".equals(message)) {
                 request.removeAttribute("modifyzjgerror");
         %>
